...
/*O ponto de partida para esta tarefa é a Tarefa de Laboratório 7 (Agenda Telefônica com Vetor de
Registros). Você deverá modificar o código desta tarefa para que haja um vetor de PONTEIROS para
registros, ao invés de um vetor de registros. Em seguida, após adaptado o código, o menu deverá
incluir uma nova opção, que é para ordenar alfabeticamente o conteúdo da agenda telefônica pelo
nome da pessoa. Utilize o algoritmo de ordenamento de sua preferência para realizar a
classificação. O ordenamento deverá ser feito exclusivamente através de manipulação de
ponteiros.*/
...
void trocar_agenda(Agenda* A, Agenda* B) {
    Agenda aux = *A;
    *A = *B;
    *B = aux;
}

void ordenar(Agenda* agenda[MAXREGISTRO], int n) {
    for(int i = 0; i < n; i++) {
        for(int j = i+1; j < n; j++) {
            if(strcmp(agenda[j]->nome, agenda[i]->nome) < 0) {
                trocar_agenda(agenda[j], agenda[i]);
            }
        }
    }
}
...
int menu(){
  ...
  printf("Digite O para ordenar nomes na agenda\n");
  scanf(" %c", &c);
  getchar();
  return c;
}
